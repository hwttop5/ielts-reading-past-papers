...
function normalizePracticeState(value: unknown, now: number): PracticeSyncState {
  ...
  const records = Array.isArray(value.records)
    ? value.records.map((entry) => normalizePracticeRecord(entry)).filter((entry): entry is PracticeRecord => Boolean(entry))
    : []
...
function mergeAchievementState(
  serverState: AchievementSyncState,
  incomingState: AchievementSyncState,
  now: number
): AchievementSyncState {
