...
async function benchmarkModel(model: string, iterations: number = 5): Promise<ModelBenchmark> {
...
describe('LLM 模型性能基准测试 - 全量模型 (5 次平均)', () => {
  it('测试所有可用模型并生成排名', async () => {
    console.log('\n' + '='.repeat(90))
    console.log('LLM 模型性能基准测试 (每个模型 5 次迭代，取平均值)')
    ...
    for (const model of MODELS_TO_TEST) {
      console.log(`\n📊 测试模型：${model}`)
      const result = await benchmarkModel(model, 5)
      benchmarks.push(result)
      console.log(`   平均：${result.avgMs.toFixed(0)}ms | 最快：${result.minMs}ms | 最慢：${result.maxMs}ms | 错误：${result.errorCount}/5`)
    }

    // 按平均速度排名
    const ranked = benchmarks
      .filter(b => b.errorCount < 3) // 排除错误率过高的模型
    ...
    ranked.forEach((bench, index) => {
      ...
      const modelDisplay = bench.model.padEnd(24)
      console.log(`${rank}\t${modelDisplay}\t${bench.avgMs.toFixed(0)}\t\t${bench.minMs}\t\t${bench.maxMs}\t\t${bench.errorCount}/5`)
    })

    // 输出所有模型成绩列表
    console.log('\n' + '='.repeat(90))
    console.log('📊 所有模型详细成绩 (按响应时间从快到慢)')
    console.log('='.repeat(90))

    // 所有模型按速度排序（包括有错误的）
    const allRanked = benchmarks.sort((a, b) => a.avgMs - b.avgMs)

    console.log('\n| 排名 | 模型 | 平均响应时间 (ms) | 最快 (ms) | 最慢 (ms) | 错误数 |')
    console.log('|------|------|------------------|-----------|-----------|--------|')

    allRanked.forEach((bench, index) => {
      const rank = index + 1
      const status = bench.errorCount > 0 ? `⚠️ ${bench.errorCount} 错误` : '✅'
      console.log(`| ${rank} | ${bench.model.padEnd(22)} | ${bench.avgMs.toFixed(0).padEnd(16)} | ${bench.minMs.toString().padEnd(9)} | ${bench.maxMs.toString().padEnd(9)} | ${status} |`)
    })
    ...
    if (ranked.length >= 2) {
      ...
      console.log('')
      console.log('建议修改 server/.env 文件:')
      console.log(`LLM_CHAT_MODEL=${ranked[0].model}`)
    }

    // 断言至少有一个模型可用
    expect(ranked.length).toBeGreaterThan(0)
  }, 600000) // 10 分钟超时（8 个模型 × 5 次 × 平均 3 秒 + 缓冲）
})
